...
static void _index (AppServer* self, WebMessage* message, GFile* root, GFile* target, GError** error)
{
                  ...
                  while (TRUE)
                    {
                      ...
                      if (info2 == NULL)
                      ...
                      else
                        {
                          ...
                          icon = g_file_info_get_is_symlink (info2) ? g_file_info_get_symbolic_icon (info2) : g_file_info_get_icon (info2);
                          ...
                          _g_free0 (href);
                          g_string_append_printf (buffer, "<img src=\"/icon/%s\"/ alt=\"[%s]\">", icondata, g_enum_get_value (klass, filetype)->value_nick);
